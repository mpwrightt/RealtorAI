...
import BuyerNotificationSettings from './notification-settings';

interface BuyerMessagesClientProps {
  sessionId: Id<"buyerSessions">;
  agentName: string;
}

export default function BuyerMessagesClient({ sessionId, agentName }: BuyerMessagesClientProps) {
  const [messageText, setMessageText] = useState('');
  const scrollRef = useRef<HTMLDivElement>(null);
  
  const messages = useQuery(api.messages.getBuyerMessages, { buyerSessionId: sessionId });
  const sendMessage = useMutation(api.messages.sendBuyerMessage);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-6">
        <BuyerNotificationSettings sessionId={sessionId} />
